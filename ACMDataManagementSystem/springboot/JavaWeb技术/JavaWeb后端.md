...
6. 前后端联调测试
#### 局域网对接前后端
1. 后端主机通过cmd命令行`ipconfig`命令获取到无线局域网适配器WLAN中的IPv4地址
2. 在后端程序配置文件中配置
3. 通过`IPv4地址:端口号/path路径`即可访问后端返回数据
#### 记录日志
...
- 会话跟踪方案：
    - 客户端会话跟踪技术：Cookie
        - 响应头set-cookie用来设置cookie数据，请求头cookie用来携带cookie的数据
        - 优点：HTTP协议中支持的技术，响应头的解析以及请求头数据的携带都是浏览器自动进行
        - 缺点：
            - 移动端APP无法使用Cookie
            - 不安全，用户可以自己禁用Cookie
            - Cookie不能跨域（跨域区分三个维度：协议、IP/域名、端口，有任意一个维度不同即为跨域操作）
    - 服务端会话跟踪技术：Session
        - 优点：存储在服务端，安全
        - 缺点：
...
- Base64：是一种基于64个可打印字符（A-Z a-z 0-9 + /）来表示二进制数据的编码方式（= 补位符号）
- 生成令牌
    - 引入JWT令牌操作工具类
    - 登陆完成后，调用工具类生成JWT令牌，并返回
#### 过滤器Filter
- 概念：Filter过滤器，是JavaWeb三大组件（Servlet、Filter、Listener监听器）之一
...
<p align="center">
    <img src="./Filter/Filter1.jpg" alt="Filter基本定义及配置" width="600">
</p>

- 执行流程：过滤器拦截到请求后首先执行放行前逻辑，执行完毕后执行放行操作，即调用`chain.doFilter(request, response)`方法执行放行操作，放行后访问对应的web资源，访问完毕后回到filter中执行放行后逻辑
